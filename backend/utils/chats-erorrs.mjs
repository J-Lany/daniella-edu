export const ERRORS = {
  getChatErrors: {
    403: "Чат не найден / недостаточно прав",
    404: "У данного пользователя нет чатов",
    500: "Ошибка в получении чатов, попробуйте позднее"
  },
  createChatErrors: {
    403: "Такой чат уже существует",
    500: "Ошибка при создании чата, попробуйте позднее"
  },
  chatErrors: {
    403: "Чат не найден / недостаточно прав",
    404: "Такого чата не существует",
    500: "Ошибка на сервере, попробуйте позднее"
  },
  registrationErrors: {
    401: "Такой пользователь уже существует",
    403: "Некорректно указан email",
    500: "Ошибка регистрации, попробуйте позднее"
  },
  loginErrors: {
    401: "Неверное имя пользователя или пароль",
    403: "Такого пользователя не существует",
    500: "Ошибка авторизации, попробуйте позднее"
  },
  tokenErrors: {
    401: "Срок действия токена обновления истек",
    500: "Ошибка обновления токена, попробуйте позднее"
  },
  userErrors: {
    401: "Такого пользователя не сущетсвует",
    403: "Пользователи не найдены",
    500: "Ошибка получения пользователей, попробуйте позднее"
  },
  updateUserErrors: {
    401: "Такого пользователя не сущетсвует",
    500: "Ошибка получения пользователей, попробуйте позднее"
  },
  messageErrors: {
    401: "В данном чате нет сообщений",
    403: "У данного автора нет соответсвующего чата",
    500: "Ошибка при работе с сообщениями, попробуйте позднее"
  }
};
