...
export class VirtualScroll extends HTMLElement {
  ...
  getAccumulatedHeight(startIndex, endIndex, index) {
    return this.itemHeights.slice(startIndex, index).reduce((sum, height) =>{ if(index > 17 ) {console.log(sum, height)}return sum + height}, 0);
    // const currentItemHeights = this.itemHeights.slice(startIndex, endIndex);
    // let accumulatedHeight = 0;
    // for (let i = 0; i < index; i++) {
    //   console.log(i, currentItemHeights[i], accumulatedHeight)
    //   console.log(currentItemHeights)
