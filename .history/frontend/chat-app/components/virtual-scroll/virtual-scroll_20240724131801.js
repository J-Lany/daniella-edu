...
export class VirtualScroll extends HTMLElement {
  ...
  getEndIndex(scrollTop) {
    ...
    for (let i = 0; i < this.itemHeights.length; i++) {
      totalHeight += this.getItemHeight(i);
      if (totalHeight > scrollTop) {
        const finalScore = i + Math.ceil(this.clientHeight / Math.min(...this.itemHeights.filter((h) => h > 0))) + this.bufferSize
  ...
  renderVisibleItems(startIndex, endIndex) {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      if (!this.visibleItems.has(i)) {
        ...
        const topPosition = this.getAccumulatedHeight(startIndex, i); 
        const testTopPosition = this.getAccumulatedHeight(0, i); 
        item.style.top = `${this.getAccumulatedHeight(0, i)}px`;
        item.style.width = "80%";
