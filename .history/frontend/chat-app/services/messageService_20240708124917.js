...
export class MessageService {
  ...
  updateMessages(chatId, newMessages) {
    ...
    newMessages.forEach(innerArray => {
      innerArray.forEach((newMessage) => {
        const existingMessageIndex = existingMessages.findIndex(existingInnerArray => {
          return existingInnerArray.some(existingMessage => existingMessage.messageId === newMessage.messageId);
        });
  
        if (existingMessageIndex !== -1) {
          const innerIndex = existingMessages[existingMessageIndex].findIndex(existingMessage => existingMessage.messageId === newMessage.messageId);
          existingMessages[existingMessageIndex][innerIndex] = newMessage;
        } else {
